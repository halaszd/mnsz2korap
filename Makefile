...
test:
	$(PYTHON3) scripts/mnsz2korapxml.py inputs/noske_test/*.mxml -m inputs/xml_clean_test/\**/\*.xml -d testKorAP -c
PHONY: test

# ----- mnsz2korap: Az MNSZ2 NoSkE formátumban lévő XML-jeit átalakítja KorAP formátumra --> éles futtatás az oliphant-on.
mnsz2korapxml:
	$(PYTHON3) scripts/mnsz2korapxml.py $(INPUT_FOLDER_NOSKE)/* -m $(INPUT_CLEAN_XMLS)  -d $(OUTPUT_FOLDER) -c
PHONY: mnsz2korapxml
